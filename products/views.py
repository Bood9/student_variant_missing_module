...
def product_list(request):
    ...
    # Фильтры и поиск доступны только администратору и менеджеру
...
def product_create(request):
    """Создание новой электрозапчасти"""
...
def product_update(request, pk):
    """Редактирование электрозапчасти"""
    role = get_user_role(request.user)
    if role not in ['admin', 'manager']:
        messages.error(request, 'У вас нет прав для выполнения этого действия.')
        return redirect('products:product_list')
...
def product_delete(request, pk):
    ...
    if product.order_set.exists():
        messages.error(request, 'Невозможно удалить товар, так как он связан с заказами.')
        return redirect('products:product_list')
